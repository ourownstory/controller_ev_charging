...
class SarsaMLPDouble(MetaController):
    def build(self):
        ...
        self.model2 = neural_network.MLPRegressor(hidden_layer_sizes=self.config.hidden_layer_sizes)
            ...
            if np.random.rand() < 0.5:
                self.model.partial_fit(fs_list,
                                       pred_fs + self.lr * masks * ((np.array(rewards) + self.gamma * np.sum(
                                           pred_fsp2 * masks_p, axis=1)).reshape(-1, 1) - pred_fs))
            else:
                self.model2.partial_fit(fs_list,
                                       pred_fs2 + self.lr * masks * ((np.array(rewards) + self.gamma * np.sum(
                                           pred_fsp * masks_p, axis=1)).reshape(-1, 1) - pred_fs2))
